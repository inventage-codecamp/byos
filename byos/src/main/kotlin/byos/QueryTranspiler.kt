...
sealed class InternalQueryNode(val graphQLFieldName: String, val graphQLAlias: String) {
    ...
    class Attribute(graphQLFieldName: String, graphQLAlias: String) :
            InternalQueryNode(graphQLFieldName, graphQLAlias)
}
...
class QueryTranspiler(
    ...
    fun buildInternalQueryTrees(
            ...
            getChildrenFromSelectionSet(queryDefinition.selectionSet, Counter()).map {
    ...
    private fun getChildrenFromSelectionSet(
            ...
            selectionSet.selections.filterIsInstance<Field>().map { selection ->
    ...
    fun resolveInternalQueryTree(
        ...
        val subSelects =
                relations.map { subRelation ->
                    val innerTable =
                            databaseMapper.getTableWithAlias(
        ...
        val providedOrderCriteria =
                (orderByArgument.firstOrNull()?.value as ObjectValue?)
                        ?.objectFields
                        ?.associate {
                            outerTable.field(it.name.lowercase())!! to (it.value as EnumValue).name
                        }
                        .orEmpty()
        val primaryKeyFields =
                outerTable.primaryKey?.fields?.map { outerTable.field(it)!! }
                        ?: outerTable.fields().toList()

        val orderByFields =
                providedOrderCriteria.keys + (primaryKeyFields - providedOrderCriteria.keys).toSet()
        val orderByFieldsWithDirection =
                orderByFields.map {
        ...
        val cursor =
                DSL.jsonObject(*orderByFields.toTypedArray())
                        .cast(String::class.java)
                        .`as`("cursor")

        val afterCondition =
                afterArgument.firstOrNull()?.let {
                    whereCondition.getForAfterArgument(it, orderByFieldsWithDirection, outerTable)
                }
                        ?: DSL.noCondition()
        ...
        val endCursorRequested =
                relation.connectionInfo
                        ?.pageInfos
                        ?.flatMap { it.endCursorGraphQlAliases }
                        ?.isNotEmpty()
                        ?: false
        val hasNextPageRequested =
                relation.connectionInfo
                        ?.pageInfos
                        ?.flatMap { it.hasNextPageGraphQlAliases }
                        ?.isNotEmpty()
                        ?: false
        ...
        val totalCountSubquery =
                DSL.selectCount().from(outerTable).where(argumentConditions).and(joinCondition)

        val endCursorSubquery =
                DSL.select(DSL.lastValue(DSL.field(cursor.name)).over()).from(cte).limit(1)
...
private fun getTableWithAlias(relation: InternalQueryNode.Relation) =
        Public.PUBLIC.getTable(relation.fieldTypeInfo.relationName)?.`as`(relation.sqlAlias)
