...
class WhereCondition(private val databaseMapper: DatabaseMapper) {
    ...
    fun getForAfterArgument(
            afterArgument: Argument,
            orderByFields: List<SortField<*>>,
    ...
    private fun getForAfterArgumentRec(
            afterValues: List<Pair<String, String>>,
            orderByFields: List<SortField<*>>,
            table: Table<out Record>
    ): Condition {
        ...
        val tail =
                afterValues.drop(1).ifEmpty {
                    return cond
                }

        return cond.or(
                head.let { (field, value) ->
                    (table.field(field) as Field<Any>)
                            .eq(value)
                            .and(getForAfterArgumentRec(tail, orderByFields, table))
                }
        )
    }
}
